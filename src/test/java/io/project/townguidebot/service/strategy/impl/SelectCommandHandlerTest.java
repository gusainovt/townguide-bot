...
class SelectCommandHandlerTest {

    @Mock
    private SendingService sendingService;

    @Test
    void getCommandType_ShouldReturnSelect() {
        SelectCommandHandler handler = new SelectCommandHandler(sendingService);

        assertEquals(CommandType.SELECT, handler.getCommandType());
    }

    @Test
    void handle_ShouldExecuteSelectCityCommand() throws Exception {
        SelectCommandHandler handler = new SelectCommandHandler(sendingService);
        TelegramLongPollingBot bot = org.mockito.Mockito.mock(TelegramLongPollingBot.class);
        SendMessage msg = new SendMessage();
        when(sendingService.selectCityCommandReceived(10L)).thenReturn(msg);
